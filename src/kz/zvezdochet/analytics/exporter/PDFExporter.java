...
//----------тигр
			...
			if (1 == id) {
				...
				if (planetText != null) {
					section.add(new Paragraph(planetText.getPlanet().getPositive() + ":", bold));
					section.add(new Paragraph(PDFUtil.html2pdf(planetText.getText())));
				}
				planetText = (PlanetText)service.findByPlanet(32L, "positive");
				if (planetText != null) {
					section.add(new Paragraph(planetText.getPlanet().getPositive() + ":", bold));
					section.add(new Paragraph(PDFUtil.html2pdf(planetText.getText())));
				}
...
//			}
	...
	private void printAspectTypes(PdfWriter writer, Chapter chapter, Event event) {
		try {
			...
			li = new ListItem();
	        li.add(new Chunk("Чем больше духовных, тем более высокого уровня развития вы достигли (больше одного — уже хорошо)", new Font(baseFont, 12, Font.NORMAL, BaseColor.MAGENTA)));
	        list.add(li);
			section.add(list);
		} catch(Exception e) {
			e.printStackTrace();
...
//			conf.setRightFoot(new Planet[] { (Planet)planetService.find(20L), (Planet)planetService.find(31L) });
//			confs.add(conf);
...
//----------|
	...
	private void printHouses(PdfWriter writer, Chapter chapter, EventStatistics statistics) {
		try {
			...
			Paragraph p = new Paragraph();
			p.add(new Chunk("Более подробно эти сферы жизни описаны в разделе ", font));
	        p.add(anchor);
			section.add(p);
		} catch(Exception e) {
			e.printStackTrace();
	...
	private void printConf(Event event, Section section, AspectConfiguration conf, Sign sign) {
		try {
			...
			String[] triangle = new String[] {"ram", "lasso", "bilasso", "poleaxe", "taucross", "triangle", "pitchfork", "bisextile", "tunnel", "dagger", "javelin", "roof", "lock", "ship", "boat", "palm", "pyramid", "compass"};
			String[] rhombus = new String[] {"rocket", "boomerang", "sail"};
			String[] tetragon = new String[] {"vehicle", "isolator", "cross", "trapezoid", "railing", "box", "stretcher", "envelope", "chimney", "perspective", "barrier"};
...
//========= треугольники
				...
				if (!code.equals("triangle") && !code.equals("bilasso")) {
...
//----------трапеция
					...
					for (Planet planet : conf.getRightFoot()) {
						PlanetText ptext = (PlanetText)ptservice.findByPlanet(planet.getId(), conf.isRightFootPositive() ? "positive" : "negative");
						if (ptext != null) {
							String s = ptext.getPlanet().getPositive();
							if (term)
								s += " (" + ptext.getPlanet().getName() + ")";
...
//========= пятиугольник
	 ...
	 * @param chapter раздел
	 * @param event событие
	 * @link http://mirkosmosa.ru/lunar-calendar/phase-moon/1989/march/3
